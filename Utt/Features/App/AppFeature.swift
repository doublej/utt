...
struct AppFeature {
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            // Disarming has to land now rather than at the next settings edit —
            // the whole point of the switch is the microphone closing, indicator
            // and all. A clip in flight is thrown away: nothing would stop it once
            // the hotkey has stopped being listened to.
            case let .settings(.dictationChanged(enabled)):
                return .merge(
                    enabled ? .none : .send(.transcription(.cancelRecording(silent: true))),
                    syncProcessor(state),
