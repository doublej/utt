...
public struct ExtensionRGB: Equatable, Hashable, Sendable {
...
/// One control on an extension's page.
public struct ExtensionSetting: Codable, Hashable, Sendable, Identifiable {
    ...
    public var value: ExtensionValue

    public init(
        key: String, kind: Kind, label: String,
        detail: String? = nil, options: [String] = [], value: ExtensionValue
    ) {
        self.key = key
        self.kind = kind
        self.label = label
    ...
    /// `options` to give, and demanding the key would reject every extension that
    /// omits it — which is every extension.
    public init(from decoder: any Decoder) throws {
        ...
        // No default is the extension declining to choose one; the kind decides what
        // an absent value means, and `sanitized()` fills it in.
        value = (try? container.decodeIfPresent(ExtensionValue.self, forKey: .value)) ?? .bool(false)
    }
    ...
    public func sanitized() -> ExtensionSetting? {
        guard ExtensionManifest.isSafeKey(key), let label = ExtensionManifest.text(label)
        else { return nil }
        let options = options
            .compactMap { ExtensionManifest.text($0, limit: 40) }
        ...
        return ExtensionSetting(
            ...
            kind: kind,
            label: label,
            detail: detail.flatMap { ExtensionManifest.text($0, limit: 160) },
            options: Array(options),
            value: value
        )
    }

    /// Whether a value the extension — or utt's own stored file — offers can be shown
    /// in this control at all.
    public func accepts(_ candidate: ExtensionValue) -> Bool {
    ...
    private func resolvedValue(options: [String]) -> ExtensionValue {
        switch (kind, value) {
...
/// so an extension reads its own values file without knowing anything about utt.
public enum ExtensionValue: Codable, Hashable, Sendable {
    case bool(Bool)
    case string(String)
    case number(Double)
